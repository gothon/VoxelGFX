...
Function VoxNewVolume(T As VoxVolumeType = 0) As Vox_Volume
    ReDim Preserve InternalVoxModels(UBound(InternalVoxModels) + 1)
    VC.CurVol = @InternalVoxModels(UBound(InternalVoxModels))
    VC.CurVol->VolType = T
    Return UBound(InternalVoxModels)
End Function

Function VoxNewVolume(Size As Vec3I, T As VoxVolumeType = 0) As Vox_Volume
    Function = VoxNewVolume(T)
    VoxSizeVolume Size
End Function

Function VoxNewVolume(SizeX As Integer, SizeY As Integer, SizeZ As Integer, T As VoxVolumeType = 0) As Vox_Volume
...
Function VoxGetVolumeSize Alias "VoxGetVolumeSize" (V As Vox_Volume = -1) As Vec3I
...
Sub VoxSetVolumeType(T As VoxVolumeType)
    With *VC.CurVol
        ...
        If .VolType = Volume_Static Then
            ...
            glBindTexture GL_TEXTURE_3D, 0
        End If
        If T <> Volume_OffScreen And .VolType = Volume_OffScreen Then
            If .Tex = 0 Then glGenTextures(1, @.Tex)
            glBindTexture GL_TEXTURE_3D, .Tex
            glTexParameteri GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, GL_CLAMP
            glTexParameteri GL_TEXTURE_3D, GL_TEXTURE_WRAP_T, GL_CLAMP
...
Sub VoxSetVolume(S As Vox_Volume)
    If S < 0 Or S > UBound(InternalVoxModels) Then Exit Sub
    VC.CurVol = @InternalVoxModels(S) 
End Sub

Sub VoxSetSource(S As Vox_Volume)
    If S < 0 Or S > UBound(InternalVoxModels) Then VC.SourceVol = NULL: Exit Sub
    VC.SourceVol = @InternalVoxModels(S) 
End Sub
...
Function VoxVolumePtr() As UInteger Ptr
    If VC.CurVol->LockedCount = 0 Then Return NULL
    Return VC.CurVol->ClientTex.A
End Function
...
Sub VoxBlit(ByVal DestV As Vec3I, ByVal SrcV As Vec3I, ByVal Size As Vec3I)
    If VC.CurVol = VC.SourceVol Or VC.SourceVol = NULL Then Exit Sub
    ...
    With *VC.SourceVol 'Clip against the source
    ...
    With *VC.CurVol 'Clip against the destination
        ...
        If IP(0) = 0 Then K = 1
        If IP(0) = 1 Then K = VC.SourceVol->W
        If IP(0) = 2 Then K = VC.SourceVol->W * VC.SourceVol->H
